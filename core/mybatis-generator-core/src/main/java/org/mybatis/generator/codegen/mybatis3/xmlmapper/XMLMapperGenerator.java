...
public class XMLMapperGenerator extends AbstractXmlGenerator {
    ...
    protected XmlElement getSqlMapElement() {
        ...
        addCondList(answer);
        addSelectByCond(answer);
        addCountByCond(answer);
        return answer;
    }

    private void addCountByCond(XmlElement answer) {
        if (introspectedTable.getRules().generateCountByCond()) {
            AbstractXmlElementGenerator elementGenerator = new CountByCondElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, answer);
        }

    }

    private void addSelectByCond(XmlElement answer) {
        if (introspectedTable.getRules().generateSelectByCond()) {
            AbstractXmlElementGenerator elementGenerator = new SelectByCondElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, answer);
        }

    }

    private void addCondList(XmlElement answer) {
        if (introspectedTable.getRules().generateCountByCond() || introspectedTable.getRules().generateSelectByCond()) {
            AbstractXmlElementGenerator elementGenerator = new CondColumnListElementGenerator();
            initializeAndExecuteGenerator(elementGenerator, answer);
        }
    }
